...
//'catList', 'cat', 'prod'
    ...
    public function rules() {
        return [
            ...
            [['parent_match_id', 'competition_id', 'status', 'completed', 'priority', 'ussd_priority'], 'integer'],
            [['start_time', 'bet_closure', 'created', 'modified', 'ussd_priority'], 'safe'],
    ...
    public function dailyShareMatches() {
        ...
        $sql = "SELECT m.game_id, CONCAT(m.home_team , ' vs ' ,m.away_team) AS teams, 
            ...
            ORDER BY FIELD(e.odd_key,1,'x',2)) AS gc,c.competition_name FROM `match` m 
            ...
            WHERE c.sport_id = 79 AND e.sub_type_id = 1 AND m.start_time >= DATE_ADD(NOW(), INTERVAL 2 HOUR)  
            AND m.start_time <= DATE_ADD(NOW(), INTERVAL 24 HOUR)
        ...
        foreach ($data as $key => $value) {
            $matches[] = $value['game_id'] . " - " . $value['teams'] . " - "
                    . $value['da'] . " - " . $value['competition_name'] . " ( " . $value['gc'] . " )";
        }
        return $matches;
    }
