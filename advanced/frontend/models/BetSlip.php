...
class BetSlip extends \yii\db\ActiveRecord {
    ...
    public function getMatch() {
        ...
        if ($this->live_bet == 1) {
            #var_dump($this->parentLiveMatch);die();
            if ($this->parentLiveMatch===NULL) {
                return $this->parentLiveMatchArch->home_team . ' VS ' . $this->parentLiveMatchArch->away_team;
            }
            return $this->parentLiveMatch->home_team . ' VS ' . $this->parentLiveMatch->away_team;
        } else {
            //OutrightCompetitor
            if ($this->outrightCompetitor===NULL) {
               return $this->parentMatch->home_team . ' VS ' . $this->parentMatch->away_team;
            }
            return $this->outrightCompetitor->competitor_name;
        }
    }
    ...
    public function getHome_team() {
        if ($this->live_bet == 1) {
            if ($this->parentLiveMatch===NULL) {
                return $this->parentLiveMatchArch->home_team;
            }
    ...
    public function getAway_team() {
        if ($this->live_bet == 1) {
            if ($this->parentLiveMatch===NULL) {
                return $this->parentLiveMatchArch->away_team;
            }           
    ...
    public function getLiveBetTime() {
        if ($this->live_bet == 1) {
            if ($this->parentLiveMatch===NULL) {
                return $this->parentLiveMatchArch->start_time;
            }
    ...
    public function getSubType() {
        ...
        return $this->outrightCompetitor->event_name;
    }
    ...
    public function getGameId() {
        if ($this->live_bet == 1) {
            if ($this->parentLiveMatch===NULL) {
                return $this->parentLiveMatchArch->game_id;
            }
    ...
    public function getOutcome() {
        if ($this->outrightCompetitor===NULL) {
            $result = Outcome::find()->where(['parent_match_id' => $this->parent_match_id, 'sub_type_id' => $this->sub_type_id, 
               'special_bet_value' => $this->special_bet_value, 'live_bet'=>$this->live_bet, 
        ...
        return $this->outrightCompetitor->result;
    }
    ...
    public function getParentLiveMatchArch() {
        return $this->hasOne(ArchLiveMatch::className(), ['parent_match_id' => 'parent_match_id']);
    }

    public function getOutrightCompetitor() {
        $result = Outright::find();
        $result->join('inner join', 'outright_competitor', 'outright.parent_outright_id = outright_competitor.parent_outright_id');
        $result->where(['outright.parent_outright_id' => $this->parent_match_id, 'outright_competitor.betradar_competitor_id' => $this->bet_pick]);
        return $result->one();
    }
        ...
        try {
            ...
            foreach ($bet_slips as $key => $bet_slip) {
             //insert into void_bet_slip 
                ...
                $vbsql = "INSERT ignore INTO void_bet_slip VALUES(0,$parent_match_id,$bet_id,"
            ...
            foreach ($parent_match_ids as $key => $parent_match_id) {
                ...
                $bsql = 'UPDATE bet b '
                        ...
                        . 'WHERE s.parent_match_id = ' . $parent_match_id . ' AND  s.sub_type_id <> -1 and s.status =1 ' . $ex_sql . $livebet_cond;
                ...
                $model->sub_type_id = -1;
                $model->special_bet_value = "";
                $model->status = 0;
                $model->created_by = Yii::$app->user->identity->username;
                $model->created = date('Y-m-d h:m:s');
                $model->winning_outcome = "-1";
                $model->is_winning_outcome="1";
                if ($model->save()) {
                    $data = ["parent_match_id"=>$parent_match_id,
                            "outcomes"=>[
...
//create outcome and push to queue, sub_type_id=-1,pick=-1
    ...
    public function matches_to_void() {
        ...
        $rows = (new \yii\db\Query())
                ...
                ->from('match')
               // ->where('start_time > ' . new \yii\db\Expression('NOW() - INTERVAL 6 Month'))
               // ->where('start_time < ' . new \yii\db\Expression('NOW()'))
               ->andWhere('status = 3') # and (select count(bet_slip_id) from bet_slip where parent_match_id=`match`.parent_match_id and status=1)>0 ')
                ->andWhere('(select parent_match_id from outcome where sub_type_id=-1 and parent_match_id=`match`.parent_match_id) is null')
                ->all();
        //die($rows->createCommand()->getRawSql());
